...
import { calculatePackersData, mapData } from "./utils/helpers.js";

const parseData = () => {
  setDefaultDateTime();
  spinner("add");
  const url = "../data/dispatch-report.csv";
  Papa.parse(url, {
    download: true,
    header: true,
    ...
    complete: function (results) {
      if (results.data.length !== 0) {
        const uniqueArr = [...new Set(results.data.map(JSON.stringify))].map(
          JSON.parse
        );
