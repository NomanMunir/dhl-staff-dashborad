import {
  perfChartCheckboxElem,
  uphChartCheckboxElem,
  fromDate,
  toDate,
...
export const charts = (packersData) => {
  ...
  document.getElementById("uph-charts").innerHTML =
    '<div class="shadow-sm m-3"><canvas id="cnv_uph_chart"></canvas></div>';
  document.getElementById("perf-charts").innerHTML =
    '<div class="shadow-sm"><canvas id="cnv_perf_chart"></canvas></div>';
  ...
  //sort by perf
  const perfPackers = perfData.sort(
    (firstPerson, secondPerson) =>
      +secondPerson["performance"] - +firstPerson["performance"]
  );

  const uphPackers = packersData.sort(
    (firstPerson, secondPerson) => +secondPerson["uph"] - +firstPerson["uph"]
  );
  //Choose Background color for the charts bar
  const backgroundColorForUph = uphPackers.map((packer) =>
    chooseColorForUphChart(packer["uph"])
  );
  const backgroundColorForPerf = perfPackers.map((packer) =>
    chooseColorForPerfChart(packer["performance"])
  );
  ...
  const formattedPackersChartDate = () => {
    return `${fromDate().toLocaleDateString(undefined, {
      ...
      hour12: false,
    })} hrs to ${toDate().toLocaleDateString(undefined, {
      day: "numeric",
      month: "long",
    })} @ ${toDate().toLocaleTimeString("en-US", { hour12: false })} hrs`;
  };
  const uphChartData = {
