...
import { fromDate, toDate } from "../components/elements.js";
...
export const mapData = (data) => {
  ...
  const result = data
    .filter(
      (items) =>
        items["Airway bill no"].trim().length !== 2 &&
        items["Order type"] !== ""
    )
    .map((data) => {
      return {
...
export const calculatePackersData = () => {
  ...
  const calculatedData = Object.entries(packersDataGroupByNames)
    .slice()
  ...
  const results = calculatedData.filter(
    (packer) => Object.keys(packer.numberOfOrders).length >= 20
  );
  window.packers = results;
  window.packersNoFilter = calculatedData.filter(
    (packer) => Object.keys(packer.numberOfOrders).length >= 10
  );
  if (results.length !== 0) {
...
export const getTotalTime = (packerData) => {
  ...
  Object.values(packerData["orders"]).forEach(
    (orderData, currentIndex, array) => {
