import { capitalize, toHoursAndMinutes } from "../utils/helpers.js";
...
export const makeTable = (packers) => {
  ...
  const grandTotalItems = packers.reduce(
    (a, v) => a + v["numberOfItems"].length,
    0
  );
  const grandTotalOrders = packers.reduce(
    (a, v) => a + Object.values(v["numberOfOrders"]).length,
    0
  );
  ...
  tableHtml.push(`
        ...
        <th>${toHoursAndMinutes(
          Math.floor(grandTotalHr / packers.length)
        )} hr</th>
  			</tr>`);
